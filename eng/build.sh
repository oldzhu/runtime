...
while [[ $# > 0 ]]; do
  ...
  case "$opt" in
     ...
     -arch|-a)
      ...
      case "$passedArch" in
        x64|x86|arm|armel|arm64|loongarch64|s390x|wasm|riscv32|riscv64)
          arch=$passedArch
          ;;
        *)
          echo "Unsupported target architecture '$2'."
          echo "The allowed values are x86, x64, arm, armel, arm64, loongarch64, s390x, riscv32, riscv64 and wasm."
          exit 1
          ;;
      esac
      shift 2
